...
public class BitmapManager {  
    ...
    public void loadBitmap(String url, ImageView imageView) {  
    	loadBitmap(url, imageView, this.defaultBmp, 0, 0, false);
    }
     ...
     * @param imageView
     */
    public void loadBitmapRefresh(String url, ImageView imageView) {  
    	loadBitmap(url, imageView, this.defaultBmp, 0, 0, true);
    }
    ...
    public void loadBitmap(String url, ImageView imageView, Bitmap defaultBmp) {  
    	loadBitmap(url, imageView, defaultBmp, 0, 0, false);
    }
     ...
     * @param height
     */
    public void loadBitmap(String url, ImageView imageView, Bitmap defaultBmp, int width, int height, boolean refresh) {  
        imageViews.put(imageView, url);  
        Bitmap bitmap = getBitmapFromCache(url);  
   
        if (bitmap != null && !refresh) {  
			//显示缓存图片
     ...
     * @param height
     */
    private Bitmap downloadBitmap(String uid, int width, int height) {   
        Bitmap bitmap = null;
        try {
			//http加载图片
			bitmap = NetApiClient.getNetBitmap(uid);
			if(width > 0 && height > 0) {
				//指定显示图片的高宽
