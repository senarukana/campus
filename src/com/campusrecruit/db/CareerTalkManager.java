...
public class CareerTalkManager implements TruncatableManager {
	...
	public void updateJoin(int careerTalkID, boolean isJoin) {
		...
		db.update(TABLE_NAME, cv, "career_talk_id=?",
				new String[] { careerTalkID + "" });
		Log.i("test","update date complete");
	}
	...
	public void addBase(CareerTalk careerTalk) {
		...
		cv.put("company_type", careerTalk.getCompanyType());
		cv.put("school_name", careerTalk.getSchoolName());
		...
		cv.put("clicks", careerTalk.getClicks());
		try {
	...
	public ArrayList<CareerTalk> getList(Cursor c) {
		Log.i("sqlite", "getCareerTalk List");
		ArrayList<CareerTalk> list = new ArrayList<CareerTalk>();
		try {
			while (c.moveToNext()) {
				...
				careerTalk.setClicks(c.getInt(c.getColumnIndex("clicks")));
	...
	public ArrayList<CareerTalk> getAllData() {
		deleteObsoleteData();
		return getList(queryAllData());
	}

	public ArrayList<CareerTalk> getFavorates(boolean flag) {
	...
	private Cursor queryFavorates(boolean flag) {
		String[] columns = new String[] { "career_talk_id", "topic_id",
				...
				"school_name", "date", "time", "place", "joins", "replies","clicks",
				"isjoined", "introduction" };
		Cursor c = db
				.query(TABLE_NAME,
						columns,
						"isjoined=?",
						new String[] { flag == true ? 1 + "" : 2 + "" }, null,
						null, "career_talk_id DESC");
		return c;
	}
